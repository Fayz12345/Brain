/****** Object:  StoredProcedure [dbo].[Report_D_DespatchNote]    Script Date: 05/27/2020 14:24:20 ******/
SET ANSI_NULLS ON
GO
...
exec Report_D_DespatchNote 'ORDER', 'B993'
...
Select * from Orderheader where OrderNumber = 'B993'
Select * from OrderCompany where OrderheaderID = 18
exec Report_D_DespatchNote 'SORDER', 'B993'
...
Declare @CompanyNameC nvarchar(50)
Select @CompanyName = '';
Select @CompanyNameC = '';
...
if @QuerryField = 'SORDER'
   ...
   SELECT OrderDetailReceiveDetail.ReceiveDetailID
	...
	Select Top 1 @CompanyName = CompanyName from OrderHeader a Inner join OrderCompany B ON a.OrderHeaderID = B.OrderHeaderID where A.OrderNumber = @Value and CompanyType = 'ShipTo'
	Select Top 1 @CompanyNameC = CompanyName from OrderHeader a Inner join OrderCompany B ON a.OrderHeaderID = B.OrderHeaderID where A.OrderNumber = @Value and CompanyType = 'Client'
    Select @CompanyName = ISNULL(@CompanyName, '')       
    Select @CompanyNameC = ISNULL(@CompanyNameC, '')           
    -- Select @CompanyName = ISNULL(@CompanyName, @CompanyNameC)
...
, #TempTable.PSlip
, Case when LEN(ISNULL(#TempTable.ShipTo, '')) = 0 then @CompanyName else #TempTable.ShipTo end as ShipTo
, ReceiveDetail.ESN
