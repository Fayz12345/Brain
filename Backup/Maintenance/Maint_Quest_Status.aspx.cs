...
{
    public partial class Maint_Quest_Status : System.Web.UI.Page
    {
        ...
        {
            clsLinqDataContext ctx = new clsLinqDataContext();
            MainGrid.DataSource = (from x in ctx.QuestionStatus.OrderBy(y => y.Status) select new { x.QuestionStatusID, x.Status });
            MainGrid.DataBind();
        }
        ...
        protected void MainGrid_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (MainGrid.SelectedIndex >= 0)
            {
                ...
                if (qu != null)
                {
                    EditKeyID.Text = qu.QuestionStatusID.ToString();
        ...
        protected void AddOK_Click(object sender, EventArgs e)
        {
            ...
            clsLinqDataContext ctx = new clsLinqDataContext();
            QuestionStatus qu = new QuestionStatus();
            ...
            ctx.QuestionStatus.InsertOnSubmit(qu);
            ctx.SubmitChanges();
            UpdateMainGrid();
            pnlMainView.Visible = true;
            pnlAdd.Visible = false;
