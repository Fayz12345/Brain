...
{
    public partial class Maint_Quest_Type : System.Web.UI.Page
    {
        ...
        {
            clsLinqDataContext ctx = new clsLinqDataContext();
            MainGrid.DataSource = (from x in ctx.QuestionTypes.OrderBy(y => y.Type) select new { x.QuestionTypeID, x.Type });
            MainGrid.DataBind();
        }
        ...
        protected void MainGrid_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (MainGrid.SelectedIndex >= 0)
            {
                ...
                clsLinqDataContext ctx = new clsLinqDataContext();
                QuestionType qu = ctx.QuestionTypes.FirstOrDefault(x => x.QuestionTypeID == KeyID);
                if (qu != null)
                {
                    EditKeyID.Text = qu.QuestionTypeID.ToString();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MainGrid.SelectedIndex >= 0)
            {
                try
                {
                    ...
                    if (qu != null)
                    {
                        ctx.QuestionTypes.DeleteOnSubmit(qu);
                        ctx.SubmitChanges();
                        UpdateMainGrid();
                    }
                }
        ...
        protected void AddOK_Click(object sender, EventArgs e)
        {
            ...
            ctx.QuestionTypes.InsertOnSubmit(qu);
            ctx.SubmitChanges();
            UpdateMainGrid();
            pnlMainView.Visible = true;
            pnlAdd.Visible = false;
