...
{
    public partial class Maint_Process : System.Web.UI.Page
    {
        ...
        protected void UpdateMainGrid()
        {
            ...
            MainGrid_B.DataSource = (from x in qm.GetProcesssDeferedThisProject(ProjectID).OrderBy(y => y.Name).OrderBy(y => y.Sequence)
                                     select new ProcessGridData(
                                         x.ScanKey,
                                         x.MacroKey,
                                         x.Name,
                                         ...
                                         x.CanJumpProject,
                                         x.isReadOnly,
                                         x.TurnStickyOn, x.DisablePrint, x.ForcePrintOnSave
                                     )).ToList();
            //pnlChild.Visible = false;
        ...
        {
            if (MainGrid_B.CurrentTable.CurrentRecord == null) { return; }
            ProcessGridData rec = (ProcessGridData)MainGrid_B.CurrentTable.CurrentRecord.GetData();
            decimal KeyID = rec.ProcessID;
            //decimal KeyID = decimal.Parse(MainGrid.SelectedValue.ToString());
        ...
        protected void UpdateEditScreen()
        {
            ...
            if (process != null)
            {
                ...
                if (process.TurnStickyOn == null || process.TurnStickyOn == false)
                {
                    EditTurnStickyOn.Checked = false;
                }
                else { EditTurnStickyOn.Checked = true; }
                if (process.AllowXBINX == null || process.AllowXBINX == false) { EditAllowXBINX.Checked = false; }
                else { EditAllowXBINX.Checked = true; }

                if (process.ShowCompletedStatus == null || process.ShowCompletedStatus == false) { EditShowCompletedStatus.Checked = false; }
                else { EditShowCompletedStatus.Checked = true; }

                if (process.isReadOnly == null || process.isReadOnly == false) { EditisReadOnly.Checked = false; }
        ...
        void btnPrintScanCodes_Click(object sender, EventArgs e)
        {
            ...
            ProcessGridData rec = (ProcessGridData)MainGrid_B.CurrentTable.CurrentRecord.GetData();
            decimal KeyID = rec.ProcessID;
            ScriptManager.RegisterStartupScript(this, GetType(), "Print", "PrintScanCodes(" + KeyID.ToString() + ");", true);
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            decimal KeyID = rec.ProcessID;
            ProcessManager pm = new ProcessManager(User.Identity.Name);
            Process OPS = pm.GetProcesss(KeyID);
            try
            {
                pm.DeleteProcess(KeyID);
    ...
    public class ProcessGridData
    {
        ...
        public bool? ForcePrintOnSave { get; set; }
        ...
        public ProcessGridData(string scankey, string macrokey, string name
            , decimal processid, string description, string description_client, string status,
              ...
              bool? showtat, bool? canjumpproject, bool? isreadonly, bool? turnstickyon, bool? disableprint, bool? forceprintonsave)
        {
            ScanKey = scankey;
            MacroKey = macrokey;
            Name = name;
            ...
            ForcePrintOnSave = forceprintonsave;
        }
