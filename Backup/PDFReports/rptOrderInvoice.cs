...
namespace BW_WebApp.PDFReports
{
    public class rptOrderInvoice
   {
        ...
        public rptOrderInvoice(string pField, string PackSlip, string LogoLocation, string Username)
        {
            //doc = PDFDoc;
            ParamField = pField;
            ParamValue = PackSlip;
        ...
        public rptOrderInvoice(PdfDocument PDFDoc, string pField, string PackSlip, string LogoLocation, string Username)
        {
            doc = PDFDoc;
            ParamField = pField;
            ParamValue = PackSlip;
        ...
        private DataTable CreateDataTableDetail()
        {
            ...
            foreach (var x in Data.OrderDetailLines.OrderBy(y => y.Desc_Text).ThenBy(y => y.Lock).ThenBy(y => y.Kit))
            {
                ...
                stable.Rows.Add(new string[] { "Devices", x.Desc_Text, x.Lock, x.Kit, string.Format("N0", x.QTY), string.Format("N2", x.UnitPrice), string.Format("N6", x.Discount), string.Format("N2", x.Total) });
            }


            stable.Rows.Add(new string[] { "", "", "", "Qty Sub", string.Format("N0", QTYTotal), "", "Subtotal $", string.Format("N2", PriceTotal) });
            stable.Rows.Add(new string[] { "", "", "", "", "", "", "Freight", string.Format("N2", Data.Freight) });
            TotalTax = (PriceTotal + Data.Freight) *Data.TaxRate;
            stable.Rows.Add(new string[] { "", "", "", "Tax Rate", string.Format("N2", Data.TaxRate * 10) + "%", "", "Tax", string.Format("N2", TotalTax) });
            GrandTotal = PriceTotal + Data.Freight + TotalTax;
            stable.Rows.Add(new string[] { "", "", "", "", "", "", "Invoice Total", string.Format("N2", GrandTotal) });
