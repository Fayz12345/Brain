...
using System.IO;
using BW_WebApp.PDFReports;

namespace BW_WebApp.Reports
{
    public partial class RPT_EXCEL_Out : System.Web.UI.Page
    {
            ...
            if (IsPostBack == false)
            {
                ...
                switch (ReportRequest.ToUpper())
                {
                    ...
                    case "THISONE":
                        ...
                        //UserName = Request.QueryString.Get("USERNAME");
                        ExportToExcel(sID, "CCGridReport");
                        break;
                    ...
                    case "IMGDOWNLOADSPREADSUMMARYDETAIL":
                        ...
                        ExportToExcel("exec GetReport_CCRunsSummaryDetail " + Parameters, "CCSpreadSUMRpt");
                    ...
                    case "IMGDOWNLOADSPREADDETAIL":
                        ...
                        ExportToExcel("exec GetReport_CCRunsSummaryDetail " + Parameters, "CCSpreadDetailRpt");
                        break;
                    ...
                    case "CCRUNDETAIL":
                        ...
                        Parameters += "\'\'";
                        //ExportCCTemplateToExcel("exec GetReport_CCRunBatchesControlSummaryDetail " + Parameters, "CCSpreadDetailRpt");
                        ExportToExcel("exec GetReport_CCRunsSummaryDetail " + Parameters, "CCRunDetailRpt");
                        break;
                    case "CCRUNSUMMARYDETAIL":
                        ...
                        Parameters += "\'\'";
                        //ExportCCTemplateToExcel("exec GetReport_CCRunBatchesControlSummaryDetail " + Parameters, "CCSpreadDetailRpt");
                        ExportToExcel("exec GetReport_CCRunsSummaryDetail " + Parameters, "CCRunSummaryDetailRpt");
                        break;
                    case "CCRUNSUMMARY":
                        sFieldList = Request.QueryString.Get("KEY");
                        sdata = sFieldList.Split(',');
                        ...
                        Parameters += "\'\'";
                        //ExportCCTemplateToExcel("exec GetReport_CCRunBatchesControlSummaryDetail " + Parameters, "CCSpreadDetailRpt");
                        ExportToExcel("exec GetReport_CCRunsSummaryDetail " + Parameters, "CCRunSummaryRpt");
                        break;
                    ...
                    case "CCTEMPLATESUMMARY":
                        ...
                        if (datafound == true)
                        {
                            ExportToExcel("exec GetReceiveDetailsInTheseSKULocationCondition_Summary " + Parameters, "CCTemplateSUMRpt");
                        }
                        break;
                    case "CCTEMPLATESUMMARYDETAIL":
                        ...
                        if (datafound == true)
                        {
                            ExportToExcel("exec GetReceiveDetailsInTheseSKULocationCondition " + Parameters, "CCTemplateSUMDetailRpt");
                        }
                        break;
                    case "CCTEMPLATEDETAIL":
                        ...
                        if (datafound == true)
                        {
                            ExportToExcel("exec GetDevicesInTheseSKULocationCondition " + Parameters, "CCTemplateDetailRpt");
                        }
                        break;
                    ...
                    case "IFSTRANSLOCATIONDEVICES":
                        string Location = Request.QueryString.Get("KEY");
                        UserName = Request.QueryString.Get("USERNAME");
                        ExportIFSDeviceAnalysis("L", Location, UserName);
                        break;
                    case "IFSTRANSSKUDEVICES":
                        string SKU = Request.QueryString.Get("KEY");
                        UserName = Request.QueryString.Get("USERNAME");
                        ExportIFSDeviceAnalysis("S", SKU, UserName);
                        break;


                    case "IFSLOCATIONDEVICES":
                        Batch = Request.QueryString.Get("KEY");
                        UserName = Request.QueryString.Get("USERNAME");
                        ExportIFSLocationCounts("D", Batch, UserName);
                        break;
                    case "IFSLOCATIONPARTS":
                        Batch = Request.QueryString.Get("KEY");
                        //UserName = Request.QueryString.Get("USERNAME");
                        ExportIFSLocationCounts("P",Batch, UserName);
                        break;
                    ...
                    case "DES":
                        ...
                        if (sID == null || sID.Length == 0) { sID = "-1"; }
                        if (decimal.TryParse(sID, out ID) == false) { ID = -1; }
                        ExportDespatchReportToExcel(ID); break;
                    case "DESPDF":
                        sID = Request.QueryString.Get("ID");
                        if (sID == null || sID.Length == 0) { sID = "-1"; }
                        if (decimal.TryParse(sID, out ID) == false) { ID = -1; }
                        ExportDespatchReportToPDF(ID); break;
                    case "DESPDF01":
                        sID = Request.QueryString.Get("ID");
                        if (sID == null || sID.Length == 0) { sID = "-1"; }
                        if (decimal.TryParse(sID, out ID) == false) { ID = -1; }
                        ExportDespatchReportToPDF01(ID); break;
                    case "DESPDF02":
                        sID = Request.QueryString.Get("ID");
                        if (sID == null || sID.Length == 0) { sID = "-1"; }
                        if (decimal.TryParse(sID, out ID) == false) { ID = -1; }
                        ExportDespatchReportToPDF02(ID); break;
                    case "DESDETAIL":
                        sID = Request.QueryString.Get("ID");
                        if (sID == null || sID.Length == 0) { sID = "-1"; }
                        if (decimal.TryParse(sID, out ID) == false) { ID = -1; }
                        ExportDespatchReportToExcelDetail(ID); break;
                    case "PAC":
                    case "SHIP":
                    case "DON":
                    case "ARC":
                    ...
                    case "BIN":
                        ...
                        string cmdString = "";
                        cmdString = "Exec GetReport_DevicesBIN '" + BinNumber + "'";
                        ExportToExcel(cmdString, "DeviceDataBIN");
        ...
        #region ExportIFSLocationCounts
        private void ExportIFSLocationCounts(string Report, string Location, string UserName)
        {
            IFSLocation Loc = new IFSLocation(Location);
            string cmdString = "";
            if (Loc.isValid == false) { return; }
            if (Report == "P")
            {
                cmdString = "GetPartsInThisLocation '" + Location + "'";
                ExportToExcel(cmdString, "PartsThisLocation." + Location);
            }
            if (Report == "D")
            {
                cmdString = "GetDevicesInThisLocation '" + Location + "'";
                ExportToExcel(cmdString, "DevicesThisLocation." + Location);
            }
        }
        #endregion
        #region ExportIFSDeviceAnalysis
        private void ExportIFSDeviceAnalysis(string Report, string SearchData, string UserName)
        {
            IFSLocation Loc = new IFSLocation(SearchData);
            string cmdString = "";
            if (Loc.isValid == false) { return; }
            if (Report == "L")             // Locations
            {
                cmdString = "Report_AnalysisDeviceLocation '" + SearchData + "'";
                ExportToExcel(cmdString, "Report_AnalysisDeviceLocation." + SearchData);
            }
            if (Report == "S")
            {
                cmdString = "Report_AnalysisSKU '" + SearchData + "'";
                ExportToExcel(cmdString, "Report_AnalysisSKU." + SearchData);
            }
        }
        ...
        private string ExportToExcel(string scmd, string fileName, List<string> fieldListToReport)
        {
            ...
            //string[] formatnumeric = {"TRANSACTIONQTY","TRANSACTIONUNITPRICE","QUANTITY","UNITPRICE","MONTHENDQTY","MONTHENDUNITPRICE", 
            //                             "ORIGINAL COST","SELLING PRICE","INTERNAL COST", "DEVICECOST",
            //                             "REPAIR FEE", "REPAIR_FEE", "UNITPRICE01", "UNITPRICE02", "UNITPRICE03", 
            //                             "UNITPRICE04", "UNITPRICE05", "UNITPRICE06", "UNITPRICE07", "UNITPRICE08", 
            //                             "UNITPRICE09", "UNITPRICE10", "TOTALUNITPRICE", "HOLDQUANTITY" };
            //string[] formatDate = { "LASTUPDATEDATE", "MONTHENDDATE", "CREATEDATE", "RECEIVEDATE" };
        ...
        void ExportPickingSlip_PG1_Header(clsOrderHeader OH, IWorksheet sheet)
        {
            ...
            if (OH.InternalNote != null) { sheet.Range[10, 1].Text = "Internal Note:" + (OH.InternalNote == null ? "" : OH.InternalNote); }
            if (OH.DeliveryNote != null) { sheet.Range[11, 1].Text = "Delivery Note:" + (OH.DeliveryNote == null ? "" : OH.DeliveryNote); }

            sheet.Range[6, 1].Text = OH.ClientCompany.CompanyName == null ? "" : OH.ClientCompany.CompanyName;
            sheet.Range[7, 1].Text = "Attention:" + (OH.ClientCompany.ContactName == null ? "" : OH.ClientCompany.ContactName);
            sheet.Range[8, 1].Text = OH.ClientCompany.AddressLine1 == null ? "" : OH.ClientCompany.AddressLine1;
            sheet.Range[9, 1].Text = OH.ClientCompany.AddressLine2 == null ? "" : OH.ClientCompany.AddressLine2;
            sheet.Range[10, 1].Text = (OH.ClientCompany.City == null ? "" : OH.ClientCompany.City) + "," + (OH.ClientCompany.StateOrProvince == null ? "" : OH.ClientCompany.StateOrProvince) + " " + (OH.ClientCompany.Country == null ? "" : OH.ClientCompany.Country);  
            sheet.Range[11, 1].Text = OH.ClientCompany.PostalCode == null ? "" : OH.ClientCompany.PostalCode ;
        ...
        void ExportPickingSlip_PG1_Detail(clsOrderDetailLine ODL, IWorksheet sheet, ref int RowStart, int RowEnd)
        {
            ...
            sheet.Range[row, 2].Text = left.ToString();
            //sheet.Range[row, 3].Text = ODL.Desc_Text;
            sheet.Range[row, 3].Text = (ODL.Desc_Text == null || ODL.Desc_Text.Length == 0) ? ODL.IFSSKU : ODL.Desc_Text.Replace(System.Environment.NewLine, " ");
        }
        #endregion

        #region Despatch Report

        string ExportDespatchReportToPDF01(decimal OrderHeaderID)
        {
            string msg = "";
            try
            {
                OrderManager om = new OrderManager(User.Identity.Name);
                string PSlip = om.GetOrderNumberFromID(OrderHeaderID);
                ...
                rptDespatchNote rpt = new rptDespatchNote(doc, "SORDER", PSlip, LogoPath(), User.Identity.Name);
                string Message = rpt.Message;
                //lblResults.Text = Message;
                doc.Save(string.Format("{0}.pdf", rpt.DefaultFileName), Page.Response, HttpReadType.Save);
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        string ExportDespatchReportToPDF02(decimal OrderHeaderID)
        {
            string msg = "";
            try
            {
                ...
                PdfDocument doc = new PdfDocument();
                rptOrderInvoice_02 rpt = new rptOrderInvoice_02(doc, "SORDER", PSlip, LogoPath(), User.Identity.Name);
                string Message = rpt.Message;
                //lblResults.Text = Message;
                doc.Save(string.Format("{0}.pdf", rpt.DefaultFileName), Page.Response, HttpReadType.Save);
            }
        ...
        string ExportDespatchReportToPDF(decimal OrderHeaderID)
        {
            string msg = "";
            try
            {
        ...
        string ExportDespatchReportToExcelDetail(decimal OrderHeaderID)
        {
            string msg = "";
            try
            {
                ...
                rptDespatchNote rpt = new rptDespatchNote("SORDER", PSlip, LogoPath(), User.Identity.Name);
        ...
        string ExportDespatchReportToExcel(decimal OrderHeaderID)
        {
            string msg = "";
            try
            {
        ...
        void ExportPackingSlip_PG1_Header(clsOrderHeader OH, IWorksheet sheet)
        {
            ...
            sheet.Range[5, 6].Text = "Order Number:" + (OH.OrderNumber == null ? "" : OH.OrderNumber);
            sheet.Range[7, 6].Text = "Customer PO:" + (OH.CustomerPO == null ? "" : OH.CustomerPO);
            sheet.Range[8, 6].Text = "Waybill Number:" + (OH.WaybillNumber == null ? "" : OH.WaybillNumber);
            sheet.Range[9, 6].Text = "Date:" + DateTime.Now.ToShortDateString();
            //sheet.Range[10, 5].Text = "Reference:";
            sheet.Range[10, 6].Text = "Reference:" + OH.MiscReference;
            sheet.Range[10, 6].CellStyle.HorizontalAlignment = ExcelHAlign.HAlignRight;


            // Bill To
            sheet.Range[12, 2].Text = OH.ClientCompany.CompanyName == null ? "" : OH.ClientCompany.CompanyName;
            sheet.Range[13, 2].Text = "Attention:" + (OH.ClientCompany.ContactName == null ? "" : OH.ClientCompany.ContactName);
            sheet.Range[14, 2].Text = OH.ClientCompany.AddressLine1 == null ? "" : OH.ClientCompany.AddressLine1;
            sheet.Range[15, 2].Text = OH.ClientCompany.AddressLine2 == null ? "" : OH.ClientCompany.AddressLine2;
            sheet.Range[16, 2].Text = (OH.ClientCompany.City == null ? "" : OH.ClientCompany.City) + "," + (OH.ClientCompany.StateOrProvince == null ? "" : OH.ClientCompany.StateOrProvince) + " " + (OH.ClientCompany.Country == null ? "" : OH.ClientCompany.Country);
            sheet.Range[17, 2].Text = OH.ClientCompany.PostalCode == null ? "" : OH.ClientCompany.PostalCode;

            // Ship To
            sheet.Range[12, 6].Text = OH.ShipToCompany.CompanyName == null ? "" : OH.ShipToCompany.CompanyName;
            sheet.Range[13, 6].Text = "Attention:" + (OH.ShipToCompany.ContactName == null ? "" : OH.ShipToCompany.ContactName);
            sheet.Range[14, 6].Text = OH.ShipToCompany.AddressLine1 == null ? "" : OH.ShipToCompany.AddressLine1;
            sheet.Range[15, 6].Text = OH.ShipToCompany.AddressLine2 == null ? "" : OH.ShipToCompany.AddressLine2;
            sheet.Range[16, 6].Text = (OH.ShipToCompany.City == null ? "" : OH.ShipToCompany.City) + "," + (OH.ShipToCompany.StateOrProvince == null ? "" : OH.ShipToCompany.StateOrProvince) + " " + (OH.ShipToCompany.Country == null ? "" : OH.ShipToCompany.Country);
            sheet.Range[17, 6].Text = OH.ShipToCompany.PostalCode == null ? "" : OH.ShipToCompany.PostalCode;

            if (OH.DeliveryNote != null) { sheet.Range[19, 1].Text = "Delivery Note:" + OH.DeliveryNote == null ? "" : OH.OrderNumber; }


        }
        int ExportPackingSlip_PG2_Header(clsOrderHeader OH, IWorksheet sheet, int RowStart)
        {
            sheet.Range[RowStart + 1, 6].Text = "Order Number:" + (OH.OrderNumber == null ? "" : OH.OrderNumber);
            sheet.Range[RowStart + 3, 6].Text = "Customer PO:" + (OH.CustomerPO == null ? "" : OH.CustomerPO);
            sheet.Range[RowStart + 4, 6].Text = "Waybill Number:" + (OH.WaybillNumber == null ? "" : OH.WaybillNumber);
        ...
        void ExportPackingSlip_PG1_Detail(clsOrderDetailLine ODL, IWorksheet sheet, ref int RowStart, int RowEnd)
        {
            ...
            foreach (clsOrderDetailLineReceive rl in ODL._OrderDetailReceiveLines)
            {
                if (LastSku.ToUpper() != rl.SKU.ToUpper())
                {
                    ...
                    sheet.Range[row, 3, row, 5].Text = (ODL.Desc_Text == null || ODL.Desc_Text.Length == 0) ? ODL.IFSSKU : ODL.Desc_Text.Replace(System.Environment.NewLine, " "); ;
                    sheet.MigrantRange[row, 3, row, 5].Merge();
