...
namespace BW_WebApp
{
    public partial class OrderEntry03 : System.Web.UI.Page
    {
        ...
        private string[] OrderDetailGridcols = { "btnEdit", "Project_ID", "Line_NO", "QTY", "UnitPrice", "QTYPacked", "Desc_Code", "Desc_Text", "OrderDetailID", "chkIsDeleted", "btnPack" };
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            btnRefeshx.Click += new EventHandler(btnRefeshx_Click);
            ...
            //    //if (User.IsInRole("Biller") == true || User.IsInRole("Administrators") || User.IsInRole("Supervisors")) { TabPanelBill.Visible = true; TabPanelDone.Visible = true; TabPanelArchive.Visible = true; }
                ...
                if (User.IsInRole("Shipper") == true || User.IsInRole("Administrators") || User.IsInRole("Supervisors") || User.IsInRole("Admin") == true) { TabPanelTrash.Visible = true; TabPanelSearch.Visible = true; }
            //    //if (User.IsInRole("Biller") == true || User.IsInRole("Administrators") || User.IsInRole("Supervisors")) { TabPanelBill.Visible = true; TabPanelDone.Visible = true; TabPanelArchive.Visible = true; }
        ...
        void btnRefeshx_Click(object sender, EventArgs e)
        {
            LoadOrderData();
        }
        ...
        void MoveDownPartNumber(string partnumber)
        {
            ...
            if (chkDropManufacturer.Checked == true) { NewDetailAttributeCode.Text = NewDetailAttributeCode.Text + " " + mManufacturer; }
            if (chkDropModel.Checked == true) { NewDetailAttributeCode.Text = NewDetailAttributeCode.Text + " " + mModel; }
            if (chkDropColour.Checked == true) { NewDetailAttributeCode.Text = NewDetailAttributeCode.Text + " " + mColour; }
        ...
        void MoveDown(DropDownList ctrl)
        {
            ...
            if (ctrl.SelectedIndex > -1)
            {
                ...
                NewDetailAttributeCode.Text = NewDetailAttributeCode.Text + " " + Text;
                QuestionManager QM = new QuestionManager(User.Identity.Name);
        ...
        private void ResetDropDowns()
        {
            ...
            QuestionManager qm = new QuestionManager(User.Identity.Name);
            FillDropBoxQuestion(drpCourier, qm.GetQuestionOptionList(ctx, "Courier Out"));
            //FillDropBoxQuestion(drpOrdertype, qm.GetQuestionOptionList(ctx, "Order Type"));
            FillDropBoxQuestion(drpOrdertype, qm.GetQuestionOptionList(ctx, "Transaction Type"));



            FillDropBoxQuestion(drpSalesPerson, qm.GetQuestionOptionList(ctx, "SalesPerson"));
            FillDropBoxQuestion(drpCurrency, qm.GetQuestionOptionList(ctx, "Currency"));
            RefreshMessage.Text = "";
        ...
        void SaveData()
        {
            ...
            if (OH.OrderNumber.Length == 0) { return; }
        ...
        void LoadOrderData()
        {
            ...
            if (status.Length > 0 && tabMain.ActiveTab.Visible == true)
            {
                ...
                else
                {
                    ...
                    //GridView2.DataSource = om.GetMasterOrderListDevices(ctx, status).Select(d => new OrderHeaderAbbrs(d.OrderHeaderID, d.OrderNumber, d.IFSOrderNo, d.MiscDesc, d.Site, d.CustomerPO, d.WayBillNumber, d.OrderDate, d.PickPackDate, d.ShippedDate, d.CreateDate)).OrderBy(x => x.CreateDate);
                }
                GridView2.DataBind();
            }
        }
        ...
        void LoadOrderHeader(decimal HeaderID)
        {
            ...
            lblAddOrder.Text = "Sales Order Number (PL#):" + OH.OrderNumber;               // +" (" + OH.IFSOrderNumber + ")";
        ...
        void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            switch (btn.CommandName.ToString().ToUpper())
            {
                ...
                case "PRINTDESPATCH":
                    if (decimal.TryParse(btn.CommandArgument, out id) == false) { id = -1; }
                    if (tabMain.ActiveTab.ID.ToUpper() == "TABPANELDONE" || tabMain.ActiveTab.ID.ToUpper() == "TABPANELSHIP")
                    {
        ...
        clsOrderHeader GatherSheetData()
        {
            ...
            OH.Tax = ID;
            if (decimal.TryParse(txtTaxRate.Text, out ID) == false) { ID = 0; }
            OH.TaxRate = ID;
