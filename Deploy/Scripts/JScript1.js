...
function RecordScanKey(pText) {
    ...
    if (newText.length > 0) {
        ...
        if (newText.substr(0, 1).toUpperCase() == '/' && newText.length == 3) { MCL('ScanKey').value = newText; return; }
        ...
        if (newText.substr(0, 5).toUpperCase() == 'XAUTHX') { SetupToAuthorize(newText.substr(6)); return; }
        if (newText.substr(0, 5).toUpperCase() == 'XBINX') { BinBulkProcess(newText.substr(5)); return; }

        var x = newText.substr(0, 6).toUpperCase();

        if (newText.substr(0, 4).toUpperCase() == 'XCLX') { ChangeclientProcess(newText.substr(4)); return; }
        if (newText.substr(0, 6).toUpperCase() == 'XDONEX') { DONEXBulkProcess(newText.substr(6)); return; }
        ...
        service.ScanCodeParse(MCL('ClientLocationID').value, 'XXX', cProcess, newText, MCL('UserName').value, MCL('StepUp').value, MCL('hdnManufacturerIDx').value, MCL('hdnModelIDx').value, onSuccess, null, null);
    }
}
...
function onSuccess(result) {
    ...
    if (B[2].toUpperCase() == 'UNKNOWN MACROKEY') { LoadScanNumber(B[3], false); return; }
...
function onSuccess_LoadReceiveDetail(B) {
    ...
    if (MCL('CurrentProcess').value.substr(0, 7).toUpperCase() == 'RECEIVE') {
        ...
        if (MCL('drpProjectList').options[IndexValue].text.toUpperCase() == 'GMP MSC') {             // Need to do this only if Approval required.
            uppdateStatusPanelError('Transfer Into GMP MSC')
            B[5] = "2";
            OpenWindowCtrl(B);
            return;
        }
    ...
    // B[5] is deduced in SQL.SP.ProcessScanCode
    if ((MCL('CurrentProcess').value.substr(0, 7).toUpperCase() == 'KITTING') && B[5] == "4") {
        OpenWindowCtrl(B);
        return;
    }
...
function displayResult(PartNumber) {
    var table = document.getElementById("PickedList");
    var row = table.insertRow(-1);
    var cell1 = row.insertCell(0);
    var cell2 = row.insertCell(1);
    // cell1.innerHTML = "<input id='Button1' type='button' onclick=\"DeleteResult('" + row.rowIndex + "');\" value='<'/>";
    cell1.innerHTML = "<input id='Button1' type='button' onclick=\"DeleteResult('" + PartNumber + "');\" value='<'/>";
...
function AddData() {
    ...
    var service = new WebServer_01();
    Timer1.Start();
    //    if (MCL('CHKOLDSAVE').checked == true) {
    //        SaveMessage += " (Old)" 
    //        var rValue = service.AddDataDetail(ds, onAddSaveSuccess, onAddSaveSuccessError);
    //    }
    //    else {
    //        if (MCL('RUNTHREADED').checked == true) { RunThreaded = "Y"; SaveMessage += " (Threaded)" }
    //        var rValue = service.AddDataDetailThreaded(ds, RunThreaded, onAddSaveSuccessThreaded, onAddSaveSuccessError);
    //    }
...
function LoadScanNumber(ScanNumber, ShowWarrantyMessage) {
    ...
    if (isReceiveScreen() == false) {
        uppdateStatusPanel('Unable to update from this process:' + cProcess);
        return;
    }
...
function RestoreSheetData(Data) {
    ...
    if (cProcess == cProcess_1 || cProcess == 'SEARCH' || cProcess_1 == 'SAVE') {
        ...
        //        alert('hdnCarrierID:' + MCL('hdnCarrierID').value);
        //        alert('hdnManufacturerID:' + MCL('hdnManufacturerID').value);
        //        alert('hdnModelID:' + MCL('hdnModelID').value);
        //        alert('hdnColourID:' + MCL('hdnColourID').value);
...
function OpenWindowCtrl(B) {
    ...
    MCL('wndESNFound').Title = B[3] + " already on file.";
