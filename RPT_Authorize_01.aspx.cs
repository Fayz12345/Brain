...
namespace BW_WebApp
{
    public partial class RPT_Authorize_01 : System.Web.UI.Page
    {
        ...
        public void LoadData(decimal ReceiveDetailAuthorizationLogID)
        {
            ...
            {
                ReceiveDetailAuthorizationLog rl = rdm.GetThisReceiveDetailAuthorizationLog(ctx, ReceiveDetailAuthorizationLogID);
                if (rl != null)
                {
                    bcBagtag.DataToEncode = "XAUTX" + ReceiveDetailAuthorizationLogID.ToString() + ":" + rl.ReceiveDetailID.ToString();
                    ReceiveDetail rd = rdm.ReceiveDetailNoRestrict(ctx, rl.ReceiveDetailID);
                    if (rd != null)
                    {

                        rowApprovedBy.Visible = false;
                        if (rl.AuthorizedBy != null && rl.AuthorizedBy.Length > 0)
                        {
                            rowApprovedBy.Visible = true;
                            lblApprovedBy.Text = "Approved by: " + rl.AuthorizedBy;
                        }
                        if (rl.DeclinedBy != null && rl.DeclinedBy.Length > 0)
                        {
                            rowApprovedBy.Visible = true;
                            lblApprovedBy.Text = "Declined by: " + rl.DeclinedBy;
                        }
                        ...
                        string Manufacturer = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Manufacturer");
                        string Model = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Model");
                        string Colour = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Colour");
                        string Carrier = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Carrier");
                        string NickName = rdm.GetCarrierMakeModelColour_NickName(ctx, Carrier, Manufacturer, Model, Colour);

                        string Note = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Unit Assessment");

                        lblESN.Text = rd.ESN;
                        lblBin.Text = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Bin");
                        lblRMA.Text = rd.RMANumber;
                        lblModel.Text = Model;
                        ldlManufacturer.Text = Manufacturer;
                        lblNickname.Text = NickName;

                        lblMSN.Text = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Serial Number");

                        chkInWarranty.SelectedIndex = 1;
                        chkExtendedWarranty.SelectedIndex = 1;
                        chkOutOfWarranty.SelectedIndex = 1;

                        string rType = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Receipt Type");
                        ...
                        lblFaultCodes.Text = rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Fault Code 1") + " " + rdm.GetReceiveDetailItem_DataElement(ctx, rd.ReceiveDetailID, "Fault Code 2");
                        ...
                        lblEstimateFee.Text = string.Format("{0:0.00}", rl.EstimateFee);
