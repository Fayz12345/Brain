...
{
    public partial class Maint_ClientLocation_Status : System.Web.UI.Page
    {
        ...
        {
            MainGrid.DataSource = (from x in ctx.ClientLocationStatus.OrderBy(y => y.Status) select new { x.ClientLocationStatusID, x.Status });
            MainGrid.DataBind();
        }
        ...
        protected void MainGrid_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (MainGrid.SelectedIndex >= 0)
            {
                ...
                using (clsLinqDataContext ctx = new clsLinqDataContext())
                {
                    ClientLocationStatus qu = ctx.ClientLocationStatus.FirstOrDefault(x => x.ClientLocationStatusID == KeyID);
                    if (qu != null)
                    {
                        EditKeyID.Text = qu.ClientLocationStatusID.ToString();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MainGrid.SelectedIndex >= 0)
            {
                ...
                using (clsLinqDataContext ctx = new clsLinqDataContext())
                {
                    ...
                    if (qu != null)
                    {
                        ctx.ClientLocationStatus.DeleteOnSubmit(qu);
                        ctx.SubmitChanges();
                        UpdateMainGrid(ctx);
                    }
                }
        ...
        protected void AddOK_Click(object sender, EventArgs e)
        {
            ...
            using (clsLinqDataContext ctx = new clsLinqDataContext())
            {
                ClientLocationStatus qu = new ClientLocationStatus();
                ...
                ctx.ClientLocationStatus.InsertOnSubmit(qu);
                ctx.SubmitChanges();
                UpdateMainGrid(ctx);
            }
            pnlMainView.Visible = true;
