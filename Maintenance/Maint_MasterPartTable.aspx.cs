...
namespace BW_WebApp.Maintenance
{
    public partial class Maint_MasterPartTable : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (User.IsInRole("Administrators") || User.IsInRole("Admin")) { btnResetPartNumbers.Visible = false; }
        ...
        void gvLocationTransferPartsFrom_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                LinkButton bLocation = (LinkButton)e.Row.FindControl("imgSelectLocation");
        ...
        void MainGridPN_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //LinkButton bPrint = (LinkButton)e.Row.FindControl("imgChangeCategory");
                LinkButton bLocation = (LinkButton)e.Row.FindControl("imgViewIFSLocations");
                LinkButton bModel = (LinkButton)e.Row.FindControl("imgChangeModel");
                LinkButton bEdit = (LinkButton)e.Row.FindControl("imgEditDetail");
                LinkButton bTransfer = (LinkButton)e.Row.FindControl("imgTransfer");
                LinkButton bDelete = (LinkButton)e.Row.FindControl("imgDelete");


                LinkButton bEditDemographics = (LinkButton)e.Row.FindControl("imgEditDemographics");
                LinkButton bAddTransaction = (LinkButton)e.Row.FindControl("imgAddTransaction");
        ...
        void GridViewDetail_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                LinkButton bEdit = (LinkButton)e.Row.FindControl("imgEditDetail");
                LinkButton bSave = (LinkButton)e.Row.FindControl("imgEditDetailSave");
                LinkButton bCancel = (LinkButton)e.Row.FindControl("imgEditDetailCancel");
        ...
        void GridViewDetail_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (btn.ID.ToUpper() == "IMGEDITDETAIL")
            {
                ...
                LinkButton bEdit = (LinkButton)row.FindControl("imgEditDetail");
                LinkButton bSave = (LinkButton)row.FindControl("imgEditDetailSave");
                LinkButton bCancel = (LinkButton)row.FindControl("imgEditDetailCancel");
        ...
        protected void LoadDropDowns(clsLinqDataContext ctx)
        {
            ...
            MasterPartManager mpm = new MasterPartManager(User.Identity.Name);
            List<MasterClass> mc = mpm.GetMasterClass();
            drpAddClass.Items.Clear();
            drpEditClass.Items.Clear();
            foreach (MasterClass o in mc)
            {
                ListItem li = new ListItem(o.Description, o.MasterClassID.ToString());
                drpAddClass.Items.Add(li);
                ListItem l1 = new ListItem(o.Description, o.MasterClassID.ToString());
                drpEditClass.Items.Add(l1);
            }
            drpAddClass.SelectedIndex = 0;
            drpEditClass.SelectedIndex = 0;
        ...
        protected void MainGrid_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (MainGrid.SelectedIndex >= 0)
            {
                ...
                if (mp != null)
                {
                    txtSelectMessage.Text = string.Format("{0} - {1}", mp.Name, mp.Description);
                    EditKeyID.Text = mp.MasterPartsID.ToString();
                    EditName.Text = mp.Name;
                    EditDesc.Text = mp.Description;
                    ListItem li = drpEditClass.Items.FindByValue(mp.MasterClassID.ToString());
                    if (li != null) { drpEditClass.SelectedIndex = drpEditClass.Items.IndexOf(li); }
                }
        ...
        protected void EditOK_Click(object sender, EventArgs e)
        {
            ...
            decimal ClassID = -1;
            if (decimal.TryParse(drpAddClass.SelectedItem.Value, out ClassID) == false) { ClassID = 0; }
        ...
        public class ExcelUploadData
        {
            ...
            public void Add(int Row, decimal MasterPartID, decimal MasterIFSLocationID, string Category, string Internal_PartNumber, string Internal_Description, string Make, string Model, string MFG_PartNumber, string sQuantity
                ...
                , decimal UnitPurchasePrice, decimal UnitPrice, List<Modeldata> MasterModelData, bool HasError, decimal ClientLocationID, decimal inwarrentyPrice, string Class, decimal ClassID)
            {
                ...
                ExcelUploadDataDetail part = RowData.FirstOrDefault(x => x.External_PartNumber.ToUpper() == Internal_PartNumber.ToUpper() && x.ClientLocationID == ClientLocationID);
                if (part != null)
                {
                    ...
                    if (part.ExternalPartDesc != Internal_Description)
                    {
                        part.WarningMessage += "IMM Descrption not consistent:" + part.ExternalPartDesc + "/" + Internal_Description + ",";
                    }
                    if (part.ControlPartnumber != MFG_PartNumber)
                    {
                        part.WarningMessage += "MFG_Partnumber not consistent:" + part.ControlPartnumber + "/" + MFG_PartNumber + ",";
                        part.HasErrors = true;
                    }
                    if (part.Make != Make)
                    {
                ...
                if (part == null)
                {
                    ...
                    part.ExternalPartDesc = Internal_Description;
                    part.External_PartNumber = Internal_PartNumber;
                    part.Make = Make;
                    part.ControlPartnumber = MFG_PartNumber;
                    part.Model = Model;
                    part.QTY = QTY;
        ...
        public class ExcelUploadDataDetail
        {
            ...
            public string External_PartNumber { get; set; }
            public string ExternalPartDesc { get; set; }
            public string Make { get; set; }
            public string Model { get; set; }
            public string ControlPartnumber { get; set; }
            ...
            public ExcelUploadDataDetail ExcelCopyRangeOptions(ExcelUploadDataDetail From)
            {
                ...
                x.ExternalPartDesc = From.ExternalPartDesc;
                x.External_PartNumber = From.External_PartNumber;
                x.Make = From.Make;
                x.ControlPartnumber = From.ControlPartnumber;
                x.Model = From.Model;
                x.QTY = From.QTY;
        ...
        private void ImportPartDetail_02(string FileName, bool AddMissingCategories)
        {
            ...
            int Internal_PartNumberCol = 3;
            int Internal_DescriptionCol = 4;
            int MakeCol = 5;
            int ModelCol = 6;
            int Control_PartnumberCol = 7;
            int QuantityCol = 8;
            ...
            string InternalPartDesc = "";
            string Make = "";
            string Model = "";
            string ControlPartnumber = "";
            string sQuantity = "";
            ...
            using (clsLinqDataContext ctx = new clsLinqDataContext())
            {
                ...
                while (sheet.Range[Row + 1, 1].Text != null && sheet.Range[Row + 1, 1].Text.Length > 0)          // Scankey 
                {
                    ...
                    InternalPartNumber = (sheet.Range[Row, Internal_PartNumberCol].Value == null ? "" : sheet.Range[Row, Internal_PartNumberCol].Value);
                    InternalPartDesc = (sheet.Range[Row, Internal_DescriptionCol].Value == null ? "" : sheet.Range[Row, Internal_DescriptionCol].Value);
                    ...
                    ControlPartnumber = (sheet.Range[Row, Control_PartnumberCol].Value == null ? "" : sheet.Range[Row, Control_PartnumberCol].Value);
                    ...
                    if (lGMPPartNumbers.Contains(InternalPartNumber))
                    {
                        WarningMessage += "IMM Partnumber found already inside file:" + InternalPartNumber;
                        hasError = true;
                    }
                    else { lGMPPartNumbers.Add(InternalPartNumber); }

                    if (lMFGPartNumbers.Contains(ControlPartnumber))
                    {
                        WarningMessage += "MFG_Partnumber found already inside file:" + ControlPartnumber;
                        hasError = true;
                    }
                    else { lMFGPartNumbers.Add(ControlPartnumber); }
                    ...
                    MasterPartsLinkTable pnum = ctx.MasterPartsLinkTables.FirstOrDefault(x => x.GMPPartNumber.ToUpper() == InternalPartNumber.ToUpper() && x.ClientLocationID == ClientLocationID);
                    if (pnum != null)
                    {
                        WarningMessage += "IMM Partnumber already on file:" + InternalPartNumber;
                        hasError = true;
                        //sheet.Range[Row, MessageCol].Value = WarningMessage;
                        //continue;
                    }
                    // Look to see if we have the part number already on file
                    pnum = ctx.MasterPartsLinkTables.FirstOrDefault(x => x.PartNumber.ToUpper() == ControlPartnumber.ToUpper() && x.ClientLocationID == ClientLocationID);
                    if (pnum != null)
                    {
                        WarningMessage += "MFG Partnumber already on file:" + InternalPartNumber;
                        hasError = true;
                        //sheet.Range[Row, MessageCol].Value = WarningMessage;
                        //continue;
                    }
                    ...
                    eData.Add(Row, MasterPartID, LocationID, Category, InternalPartNumber, InternalPartDesc, Make, Model, ControlPartnumber, sQuantity, sUnitPurchasePrice, sUnitPrice, WarningMessage, RecordMessage, QTY, UnitPurchasePrice, UnitPrice, MasterModelData, hasError, ClientLocationID, inWarrentyPrice, Class, ClassID);
                }
                #endregion
                #region Process Data
                //MasterPartManager mpm = new MasterPartManager(User.Identity.Name);
                ...
                foreach (ExcelUploadDataDetail ed in eData.RowData)
                {
                    if (ed.HasErrors == false)
                    {
                        ...
                        mpm.InsertPartNumber(ctx, ed.MasterPartID, -1, ClientLocationID, "-1", ed.Make, model, ed.ControlPartnumber, ed.External_PartNumber, ed.ExternalPartDesc, ed.ClassID);
                        if (type != null)
                        {
                            ed.WarningMessage += " " + mpm.AddToInventory(ctx, ed.ControlPartnumber, ed.LocationID, ed.QTY, ed.UnitPurchasePrice, ed.UnitPrice, ed.InWarrentyPrice, type.PartNumberBucketInventorysourceTypeID, "Newpart Upload", ClientLocationID, "", "", "", "", "");
                        }
                    }
            ...
            using (clsLinqDataContext ctx = new clsLinqDataContext())
            {
                ...
                if (parts != null)
                {
                    ...
                    //foreach (MasterPartsLinkTable l in ctx.MasterPartsLinkTables.OrderBy(x => x.MasterPart.Description).ThenBy(x => x.PartNumber))
                    {
                        //foreach (var zz in l.
                        Row++;
                        sheet.Range[Row, 1].Text = "WHS 001";
