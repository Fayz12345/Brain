...
namespace GMPI_WebApp.Classes
{
    public class IMEIUploadProcessor
    {
        ...
        public IMEIUploadProcessor(string DrivePathFileNameXLS_toUpload, string UserName, decimal ProjectID, decimal ClientLocationID, List<string> AttributeCloneList, clsLog log, bool ForceIMEI15)
        {
            this.log = log;
        ...
        private string LoadSeedData(IWorksheet sheet)
        {
            ...
            using (clsLinqDataContext ctx = new clsLinqDataContext())
            {
                ...
                for (int row = 2; row < 100000; row++)
                {
                    ...
                    RowData rd = new RowData(projectID, clientlocationid, saveprocessID, receiveprocessID, projectname, saveprocessName, receiveprocessName, forceIMEI15);
        ...
        private string LoadData(IWorksheet sheet)
        {
            ...
            using (clsLinqDataContext ctx = new clsLinqDataContext())
            {
                ...
                for (int row = 2; row < 100000; row++)
                {
                    ...
                    //foreach (CellData d in rd.attributeList.Where(x=> x.QuestionName.Length > 0))
                    //{
                    //    if (d.QuestionName.Length > 0)
                    //    {
                    //        ErrorText += d.QuestionName + "/";
                    //    }
                    //}

                    if (rd.IsCarrierManufacturerCombo(ctx, (attributeclonelist2.Count > 0)) == false)
                    {
                        ErrorText = "Invalid Carrier/Manufacturer/Model/Colour Combo";
                    }

                    if (ErrorText.Length == 0)
                    {
                        rdetail = rd.Save(ctx, username);
                        if (rdetail == null) { ErrorText = "IMEI found! " + ErrorText; }
                    }
                    else
                    {
                ...
                //using (clsLinqDataContext ctx = new clsLinqDataContext())
                //{
                // We need to advance any "exisiting" ESN Numbers -- But only once
                //List<string> bumpedList = new List<string>();
                foreach (ReceiveHeader rh in rhList)
                {
                    string EList = "";
                    foreach (ReceiveDetail rdx in rh.ReceiveDetails.OrderByDescending(x => x.Version))
                ...
                // we now need to syncronize the header carrier, manufacturer, model, colour and grade, Condition to the attribute below.
                foreach (ReceiveHeader rh in rhList)
                {
                    foreach (ReceiveDetail rdx in rh.ReceiveDetails.OrderByDescending(x => x.Version))
                    {
                        ctx.Utility_RebuildHeaderAttributes_ThisIMEI(rdx.ESN);
                    }
                }
            }
            return rvalue;
        ...
        private string LoadHeaderData(IWorksheet sheet)
        {
            ...
            List<string> ESNDetail = new List<string>(new string[] { "ESN", "STARTPROCESSNAME", "CURRENTPROCESSNAME", "RECEIVEDETAILID", "CLIENTLOCATIONID", "CLIENTLOCATION", "RMANUMBER", "PROJECTTAG", "PROJECTNAME", "LOCATION DEALER CODE", "VERSION" });
    ...
    public class RowData
    {
        ...
        decimal CarrierID { get; set; }
        decimal ManufacturerID { get; set; }
        decimal ModelID { get; set; }
        decimal ColourID { get; set; }
        ...
        public RowData(decimal ProjectID, decimal ClientLocationID, decimal SaveProcessID, decimal ReceiveProcessID, string ProjectName, string SaveProcessName, string ReceiveProcessName, bool Force15IMEI)
        {
            projectID = ProjectID;
            clientlocationID = ClientLocationID;
        ...
        public ReceiveDetail Save(clsLinqDataContext ctx, string username)
        {
