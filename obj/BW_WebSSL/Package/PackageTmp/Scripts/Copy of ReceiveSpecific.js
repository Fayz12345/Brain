...
function isInRole(Role) {
    ...
    if (dta.indexOf("," + Role.toUpperCase() + ",") > 0) { return true; }
    return false;
}
        ...
        if (cBox.type == 'text') {
            ...
            if (xName.indexOf('ScanKey') == -1) {
                ...
                if (PNumbers.indexOf(currentValue) > -1) {
                    ...
                    uppdateStatusPanel('IFS PO Unit Cost added:' + POUnitCost);
                    saved = true;
                    break;
                    //                    }
                }
...
function FillDropDown(DropDownName) {
    var service = new WebServer_01();
    alert('Inside the Fill Drop Downs:' + DropDownName);
    ...
    if (DropDownName == 'Model') {
        ...
        if (ctr == null) { return; }
        var rValue = service.GetMemoryDropDownData(GetDropDownValue(MCL('hdnModelID').value), MCL('UserName').value, onFillColourList, null, null);
        return;
    }
